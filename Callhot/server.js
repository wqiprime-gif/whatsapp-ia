...
// --- API CALLS ---
// --- API CALLS (bot ZapManager — sem cookie) ---
const CALLHOT_BOT_SECRET = process.env.CALLHOT_BOT_SECRET || '';

function requireBotSecret(req, res, next) {
  const secret = req.headers['x-callhot-secret'];
  if (!CALLHOT_BOT_SECRET || secret !== CALLHOT_BOT_SECRET) {
    return res.status(401).json({ error: 'Secret inválido' });
  }
  req.userId = req.body?.ownerUserId || 'zapmanager-bot';
  next();
}

function createCallRecord({ videoUrl, callerName, callerAvatarUrl, title, expectedAmount, ownerUserId }) {
  const callId = uuidv4();
  ...
  calls.set(callId, {
    ...
    expiresAt: null,
    expectedAmount: amt,
    ownerUserId: ownerUserId || 'zapmanager-bot',
    hostId: null,
    guests: new Set(),
    createdAt: new Date()
  });
  persistCalls();
  appendEvent({ id: uuidv4(), type: 'call_created', callId, at: new Date().toISOString(), userId: ownerUserId || null });
  if (amt) addSale({ callId, amount: amt, note: 'Venda via ZapManager', userId: ownerUserId || null });
  return { callId, ringUrl: `/ring/${callId}` };
}

app.post('/api/bot/create-call', requireBotSecret, (req, res) => {
  ...
  const { callId, ringUrl } = createCallRecord({
    videoUrl,
    callerName,
    callerAvatarUrl,
    title,
  ...
  res.json({ callId, ringUrl, fullRingUrl: `${publicBase}${ringUrl}` });
});
